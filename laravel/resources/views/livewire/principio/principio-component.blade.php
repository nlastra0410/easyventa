<div>
    <x-card cardTitle="Listado de Principio Activo ({{$this->totalRegistros}})">
        <x-slot:cardTools>
        ...
        <x-table>
            <x-slot:thead>
                ...
                <tr>
                    ...
                    <td>{{$Category->name}}</td>
                    <td>
                        <a href="{{route('principio.show',$Category)}}" class="btn btn-sm" title="Ver" style="border-radius:8px; background-color: #69C181; color: white">
...
<x-modal modalId="modalCategory" modalTitle="Nuevo Principio Activo">

    <form wire:submit={{$Id==0 ? "store" : "update($Id)"}}>
        <div class="form-row">
            <div class="form-group col-12">
                <label for="name">Nombre:</label>
                <input wire:model='name' type="text" class="form-control" placeholder="Nombre Principio Activo" id="name" style="border-radius: 14px">
                @error('name')
