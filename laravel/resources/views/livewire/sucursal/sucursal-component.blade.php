<div>
    <x-card cardTitle="Listado de Sucursales ({{$this->totalRegistros}})">
        <x-slot:cardTools>
        ...
        <x-table>
            <x-slot:thead>
                ...
                <tr>
                    ...
                    <td>{{$Category->name}}</td>
                    <td>
                        <a href="{{route('sucursal.show',$Category)}}" class="btn btn-sm" title="Ver" style="border-radius:8px; background-color: #69C181; color: white">
...
<x-modal modalId="modalCategory" modalTitle="Nueva Sucursal">

    <form wire:submit={{$Id==0 ? "store" : "update($Id)"}}>
        <div class="form-row">
            <div class="form-group col-12">
                <label for="name">Nombre:</label>
                <input wire:model='name' type="text" class="form-control" placeholder="Nombre sucursal" id="name" style="border-radius: 14px">
                @error('name')
